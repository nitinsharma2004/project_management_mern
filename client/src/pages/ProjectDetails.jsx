...
const ProjectDetails = () => {
  ...
  const handleDeleteIdea = (index) => {
    const updatedIdeas = ideas.filter((_, id) => id !== index);
  ...
  const getProjectWorks = async (id) => {
   await  getWorks(id, token)
      .then((res) => {
        const { backdata, status } = res.data;
        setWorks(backdata);
        setstatus(status);
            ...
            <HrHor />
            {
               status!== "Completed"?(
                ...
                <SubCards>
                  ...
                  <Ideas>
                    {ideas.map((i, id) => (
                      <IdeaCard idea={i} no={id} key={id} onDelete={() => handleDeleteIdea(id)} />
                    ))}
                  </Ideas>
                </SubCards>
              </Extra>
